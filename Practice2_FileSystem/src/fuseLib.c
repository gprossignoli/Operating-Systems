...
static int my_unlink ( const char * path )
{
	...
	if(resizeNode(idxNode,0) == -EIO)
	{
		perror("fallo al truncar el tamaño del nodo");
		return -1;
	}
	...
	{
		perror("fallo al actualizar el directorio");
		return -1;
	}
	//Actualizar el nodo -i en el disco virtual
	if(updateNode(&myFileSystem,idxNode,myFileSystem.nodes[idxNode])==-1)
	{
		perror("fallo al actualizar el directorio");
		return -1;
	}
	...
	if(updateBitmap(&myFileSystem)==-1)
	{
		perror ("Fallo al actualizar la tabla");
		return -1;
	}
		...
		, off_t offset , struct fuse_file_info *fi)
{
	...
	if(node->fileSize < offset){
		perror("offset sale fuera del fichero");
		return -1;
	}
